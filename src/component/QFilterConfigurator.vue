          ...
          <div class="col">
            ...
            <q-list dense>
              <q-scroll-area style="height: 250px">
                <q-expansion-item v-for="({ model, multiple, range, label, options }, indexF) in validFilters(validCategories[category].filters)" :default-opened="indexF === 0"
                ...
                <q-expansion-item v-for="{ model, label } in rangeFilters(validCategories[category].filters)" dense-toggle dense
                  :header-class="{ [`text-${color} bg-grey-1`]: isSetted({ model, range: true }), 'q-px-sm': true }" :key="`filter-${model}`" expand-icon-class="custom-toggle">
...
export default {
  ...
  computed: {
    ...
    isSetted () {
      return ({ model, multiple, range }) => multiple
        ...
        : (
          range
            ? !!this.myModel[model].min && !!this.myModel[model].max
            : !!this.myModel[model]
        )
    },
    settedValues () {
  ...
  methods: {
    ...
    ok () {
      console.log('ok', Object.fromEntries(this.settedValues))
      this.$emit('input', { ...Object.fromEntries(this.settedValues) })
    }
  }
}
</script>
